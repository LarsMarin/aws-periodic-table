...
# Generiere die Daten für verschiedene Quellen
echo "Generiere Daten für Web Scraping..."
PERIODIC_DATA_SOURCE=scrape PERIODIC_PRODUCTS_SIZE=300 python3 periodic.py
mv ../output.html "$OUTPUT_DIR/periodic_scrape.html"

echo "Generiere Daten für Directory API..."
PERIODIC_DATA_SOURCE=directory PERIODIC_PRODUCTS_SIZE=300 python3 periodic.py
mv ../output.html "$OUTPUT_DIR/periodic_directory.html"

echo "Generiere Daten für Merged Source..."
PERIODIC_DATA_SOURCE=merged PERIODIC_PRODUCTS_SIZE=300 python3 periodic.py
mv ../output.html "$OUTPUT_DIR/periodic_merged.html"

# Integriere Tab-Navigation
cat > integrate_tabs_multi.py << 'EOL'
#!/usr/bin/env python3
import os
import re
...
# Führe das Tab-Integrationsskript aus
python3 integrate_tabs_multi.py

echo "Fertig! HTML-Dateien mit Tab-Navigation wurden im Verzeichnis $OUTPUT_DIR erstellt."